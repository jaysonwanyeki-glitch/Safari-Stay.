...
function MapSkeleton() {
  return <div className="h-full w-full animate-pulse bg-sand-200" />;
}
  ...
  return (
    <>
      ...
      <aside className="sticky top-24 hidden h-[78vh] xl:block">
        <div className="h-full w-full overflow-hidden rounded-2xl border border-sand-200 shadow-sm">
      ...
      {show && (
        <div className="fixed inset-0 z-[55] bg-white xl:hidden">
          <div className="flex items-center justify-between border-b border-sand-200 px-4 py-3">
            ...
            <button onClick={() => setShow(false)} className="rounded-full px-3 py-1 text-sm font-semibold hover:bg-sand-100">
              Close
            </button>
          </div>
