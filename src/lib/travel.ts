...
export type MajorStage = {
  ...
  fare: string;
  /** Approximate town coordinates — power the stages map. */
  coords: { lat: number; lng: number };
};

export const MAJOR_STAGES: MajorStage[] = [
  { town: "Nairobi", county: "Nairobi", stage: "Accra Rd / Railways Bus Station / Machakos Country Bus (MCS)", routes: "Every county: Western, Rift, Eastern, Coast and Northern routes all leave from the CBD terminals", operators: ["Easy Coach", "Modern Coast", "Mash Poa", "Crown Bus", "Tawakal", "all SACCO matatus"], fare: "—", coords: { lat: -1.29, lng: 36.82 } },
  { town: "Mombasa", county: "Mombasa", stage: "Mombasa CBD (Kwa Shibu / station area)", routes: "Malindi & the north coast, Nairobi, Diani (via Likoni ferry), Ukunda", operators: ["Modern Coast", "Mash Poa", "Tahmeed", "SGR"], fare: "Bus from Nairobi ≈ KES 2,000–3,000 · SGR from ≈ KES 700", coords: { lat: -4.04, lng: 39.66 } },
  { town: "Kisumu", county: "Kisumu", stage: "Kisumu Bus Park", routes: "Kakamega, Kericho, Eldoret, Nairobi, Busia & the Uganda border, Lake Victoria towns", operators: ["Easy Coach", "Modern Coast", "Trinity Express", "SGR"], fare: "Bus from Nairobi ≈ KES 1,500–2,500", coords: { lat: -0.09, lng: 34.77 } },
  { town: "Eldoret", county: "Uasin Gishu", stage: "Eldoret Bus Park (Uganda Rd)", routes: "Kitale, Kisumu, Nairobi, Bungoma, Busia, Uganda", operators: ["Easy Coach", "Crown Bus", "North Rift Shuttle"], fare: "Bus from Nairobi ≈ KES 1,500–2,500", coords: { lat: 0.51, lng: 35.27 } },
  { town: "Kitale", county: "Trans Nzoia", stage: "Kitale Bus Park", routes: "Eldoret, Nairobi, Mt Elgon & Saiwa Swamp (matatus to the parks), Uganda border", operators: ["Easy Coach", "Crown Bus", "North Rift Shuttle", "Eldoret–Kitale matatus"], fare: "Bus from Nairobi ≈ KES 1,800–2,800", coords: { lat: 1.02, lng: 35.0 } },
  { town: "Kakamega", county: "Kakamega", stage: "Kakamega Bus Park", routes: "Kisumu, Nairobi, Mumias, the forest gate (boda)", operators: ["Easy Coach", "Modern Coast", "Kisumu–Kakamega matatus"], fare: "Bus from Nairobi ≈ KES 1,500–2,500", coords: { lat: 0.28, lng: 34.75 } },
  { town: "Kericho", county: "Kericho", stage: "Kericho Bus Park", routes: "Nairobi, Kisumu, Nakuru, the tea estates, Mau Forest & Lake Victoria", operators: ["Easy Coach", "Kericho Line"], fare: "Bus from Nairobi ≈ KES 1,200–1,800", coords: { lat: -0.37, lng: 35.28 } },
  { town: "Nakuru", county: "Nakuru", stage: "Nakuru Bus Park", routes: "Nairobi, Naivasha, Kericho, Kisumu, Eldoret, Nyahururu, Baringo & the north", operators: ["Easy Coach", "KAVS", "Nairobi–Nakuru matatus"], fare: "≈ KES 800–1,200 from Nairobi", coords: { lat: -0.3, lng: 36.08 } },
  { town: "Naivasha", county: "Nakuru", stage: "Naivasha town stage", routes: "Nairobi, Nakuru, Hells Gate & Crescent Island (boda), Mai Mahiu", operators: ["Naivasha matatus", "Easy Coach"], fare: "≈ KES 600–800 from Nairobi", coords: { lat: -0.72, lng: 36.43 } },
  { town: "Nyahururu", county: "Nyandarua", stage: "Nyahururu town stage", routes: "Nairobi (via Naivasha–Gilgil or Nyeri), Nyeri, Nakuru, Thomson's Falls (walk/boda)", operators: ["Nairobi–Nyahururu matatus", "Nyandarua shuttles"], fare: "≈ KES 700–1,000 from Nairobi", coords: { lat: 0.04, lng: 36.37 } },
  { town: "Nyeri", county: "Nyeri", stage: "Nyeri Bus Park", routes: "Nairobi, Nanyuki, Karatina, Embu, Naro Moru & Mt Kenya gates", operators: ["Nairobi–Nyeri matatus"], fare: "≈ KES 700–1,000 from Nairobi", coords: { lat: -0.42, lng: 36.95 } },
  { town: "Nanyuki", county: "Laikipia", stage: "Nanyuki town stage", routes: "Nairobi, Isiolo, Meru, Naro Moru & Mt Kenya gates, Samburu", operators: ["Nairobi–Nanyuki matatus", "Nanyuki Express", "SGR commuter"], fare: "≈ KES 800–1,200 from Nairobi", coords: { lat: 0.01, lng: 37.07 } },
  { town: "Isiolo", county: "Isiolo", stage: "Isiolo town stage", routes: "THE northern gateway: Nanyuki, Meru, Samburu, Marsabit, Moyale & the Ethiopia border", operators: ["Nairobi–Isiolo matatus", "north-bound shuttles"], fare: "≈ KES 1,000–1,500 from Nairobi", coords: { lat: 0.35, lng: 37.58 } },
  { town: "Meru", county: "Meru", stage: "Meru Bus Park", routes: "Nairobi, Embu, Nanyuki, Isiolo, Mt Kenya eastern gates", operators: ["Nairobi–Meru matatus", "Meru Express"], fare: "≈ KES 1,000–1,400 from Nairobi", coords: { lat: 0.05, lng: 37.65 } },
  { town: "Embu", county: "Embu", stage: "Embu Bus Park", routes: "Nairobi, Meru, Nyeri, Seven Forks dams (boda), Mt Kenya eastern routes", operators: ["Nairobi–Embu matatus", "Embu shuttle"], fare: "≈ KES 800–1,200 from Nairobi", coords: { lat: -0.53, lng: 37.45 } },
  { town: "Machakos", county: "Machakos", stage: "Machakos Country Bus Station (MCS), Nairobi + Machakos town stage", routes: "Nairobi (MCS is the main terminal), Kangundo, Kitui, Wote, Kajiado", operators: ["MCS–Machakos matatus"], fare: "≈ KES 300–400 from Nairobi", coords: { lat: -1.52, lng: 37.27 } },
  { town: "Thika", county: "Kiambu", stage: "Thika Bus Terminal", routes: "Nairobi (45 min), Garissa & the eastern highway, Nyeri, Embu, Meru", operators: ["Thika Rd matatus"], fare: "≈ KES 150–250 from Nairobi", coords: { lat: -1.04, lng: 37.08 } },
  { town: "Garissa", county: "Garissa", stage: "Garissa town stage", routes: "Nairobi (via Thika–Garissa Hwy), Dadaab, Wajir, the north-eastern frontier", operators: ["Tawakal Express", "Rhamu Express", "Garissa shuttles"], fare: "≈ KES 2,000–3,000 from Nairobi", coords: { lat: -0.45, lng: 39.65 } },
  { town: "Wajir", county: "Wajir", stage: "Wajir town stage", routes: "Nairobi (via Garissa), Mandera, the Kenya–Somalia frontier", operators: ["Tawakal Express", "Rhamu Express", "Garissa–Wajir shuttles"], fare: "≈ KES 3,000–4,000 from Nairobi", coords: { lat: 1.75, lng: 40.06 } },
  { town: "Kisii", county: "Kisii", stage: "Kisii Bus Park", routes: "Nairobi, Kericho, Kisumu, Migori, the Lake Victoria towns, Tabaka soapstone country", operators: ["Easy Coach", "Kisii Express", "Nairobi–Kisii matatus"], fare: "≈ KES 1,500–2,500 from Nairobi", coords: { lat: -0.68, lng: 34.77 } },
  { town: "Kapsabet", county: "Nandi", stage: "Kapsabet town stage", routes: "Eldoret, Nairobi, Kisumu, Kabartonjo, the Nandi Hills forest", operators: ["North Rift Shuttle", "Eldoret–Kapsabet matatus"], fare: "≈ KES 1,500–2,500 from Nairobi", coords: { lat: 0.2, lng: 35.1 } },
  { town: "Lodwar", county: "Turkana", stage: "Lodwar town stage", routes: "THE north-western gateway: Kitale, Eldoret, Nairobi, Kalokol, Eliye Springs, Kakuma & the South Sudan border", operators: ["North Rift Shuttle", "Eldoret–Kitale–Lodwar buses"], fare: "≈ KES 2,500–3,500 from Nairobi", coords: { lat: 3.12, lng: 35.6 } },
  { town: "Kilifi", county: "Kilifi", stage: "Kilifi town (Mombasa–Malindi Rd)", routes: "Mombasa, Malindi, Watamu, Kilifi creek & Mnarani ruins (boda)", operators: ["Mombasa–Kilifi matatus", "coast buses alight at Kilifi"], fare: "≈ KES 150–250 from Mombasa", coords: { lat: -3.63, lng: 39.85 } },
  { town: "Watamu", county: "Kilifi", stage: "Watamu junction (Mombasa–Malindi Rd)", routes: "Mombasa, Malindi, the marine park & turtle beaches (boda)", operators: ["Mombasa–Watamu matatus"], fare: "≈ KES 400–600 from Mombasa", coords: { lat: -3.35, lng: 40.03 } },
  { town: "Malindi", county: "Kilifi", stage: "Malindi town bus stop", routes: "Mombasa, Nairobi, Watamu, Gede ruins, Lamu (via Mokowe)", operators: ["Modern Coast", "Mash Poa", "Mombasa–Malindi matatus"], fare: "≈ KES 400–600 from Mombasa · ≈ KES 2,200–3,200 from Nairobi", coords: { lat: -3.21, lng: 40.12 } },
  { town: "Lamu", county: "Lamu", stage: "Mokowe jetty → Lamu town jetty", routes: "Buses run to Mokowe only — the speedboat/ferry crosses to Lamu town; flights land at Manda", operators: ["Modern Coast", "Mash Poa", "Tahmeed", "Safarilink/AirKenya"], fare: "Bus ≈ KES 2,500–3,500 + boat ≈ KES 500–1,000 · flight from ≈ KES 22,000", coords: { lat: -2.27, lng: 40.9 } },
  { town: "Diani / Ukunda", county: "Kwale", stage: "Ukunda (Diani) — via Likoni ferry or Ukunda Airstrip", routes: "Mombasa (ferry + matatu), Nairobi (bus/SGR), the south coast beaches", operators: ["Modern Coast", "Mash Poa", "SGR", "Safarilink/AirKenya"], fare: "Bus from Nairobi ≈ KES 2,000–3,000 · SGR from ≈ KES 700 · flight from ≈ KES 18,000", coords: { lat: -4.29, lng: 39.58 } },
];

/** Anchor id for a stage row — used by the map's "jump to" links. */
export function stageAnchor(town: string): string {
  return `stage-${town.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")}`;
}
